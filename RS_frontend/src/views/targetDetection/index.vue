  ...
  <div class="main">
    <el-card>
      ...
      <el-upload
        ...
        <div slot="tip" class="el-upload__tip">点击上传图片进行目标检测</div>
      </el-upload>
...
import { targetDetectionAPI } from "@/api/api";
  ...
  methods: {
    ...
    async httpRequest(file) {
      ...
      if (this.formdata.getAll("picture").length == 1) {
        ...
        targetDetectionAPI(this.formdata)
          .then((response) => {
            this.$message({
              type: "success",
              duration: 2 * 1000,
    ...
    changeBatch() {
      this.$router.push("targetDetectionbatch");
    },
  },
};
</script>
