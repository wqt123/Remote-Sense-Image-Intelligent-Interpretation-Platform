  ...
  <div class="main">
    <el-card>
      ...
      <el-upload
        ...
        <div slot="tip" class="el-upload__tip">点击上传图片进行地物检测</div>
      </el-upload>
      <el-divider></el-divider>
      <div v-show="isUploading">
        <Spinner size="huge" message="Loading..."></Spinner>
      </div>
      <div v-if="success" class="Rate"><b>物地比:&emsp;</b>{{ rate }}</div>
...
import { terrianClassficationAPI } from "@/api/api";
  ...
  methods: {
    ...
    httpRequest(file) {
      ...
      if (this.formdata.getAll("picture").length == 1) {
        ...
        terrianClassficationAPI(this.formdata)
          .then((response) => {
            this.$message({
              type: "success",
              duration: 2 * 1000,
            ...
            let ground = response.data.rate;
            this.rate = 1;
            if (ground == 1) {
              ground = 0;
            } else {
              ground = ground / (1 - ground);
              this.rate += ":" + Math.round(ground * 100) / 100;
            }
          })
          .catch((error) => {
            console.log(error);
    ...
    changeBatch() {
      this.$router.push("terrianClassificationbatch");
    },
  },
};
</script>
