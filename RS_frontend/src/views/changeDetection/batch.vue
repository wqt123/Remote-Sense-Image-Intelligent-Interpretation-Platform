...
import { changeDetectionBatchAPI, create } from "@/api/api";
...
export default {
  ...
  methods: {
    ...
    async httpRequest(file) {
      ...
      this.num = 1;
      let client = Client();
      let fname1 = `${new Date().getTime()}.zip`; //自定义文件名
      let fname2 = `${new Date().getTime()}.zip`;
      ...
      let options1 = {
        ...
        // 设置并发上传的分片数量。
        parallel: Math.ceil(
          Math.floor((this.filelist[0].size / 1024 / 1024) * 100) / 100
        ),
      ...
      let options2 = {
        ...
        // 设置并发上传的分片数量。
        parallel: Math.ceil(
          Math.floor((this.filelist[1].size / 1024 / 1024) * 100) / 100
        ),
        // 设置分片大小。默认值为1 MB，最小值为100 KB。
        partSize: 1024 * 1024,
        // headers,
      ...
      try {
        ...
        const result1 = await client.multipartUpload(
          fname1,
          this.filelist[0].raw,
          {
            ...options1,
        ...
        const result2 = await client.multipartUpload(
          fname2,
          this.filelist[1].raw,
          {
            ...options2,
        ...
        let data = {
          ...
          size:
            Math.floor(
              ((this.filelist[0].size + this.filelist[1].size) / 1024 / 1024) *
                100
            ) /
              100 +
            "M",
        ...
        create(data)
          .then((response) => {
            ...
            changeDetectionBatchAPI(response.data)
              .then((res) => {
                console.log(res);
                this.filelist = [];
                this.num = 0;
    ...
    changeBatch() {
      this.$router.push("changeDetection");
    },
  },
};
</script>
